...
function MainCtrl($http, $rootScope) {
    ...
    $rootScope.bebe = {}
    $rootScope.logado = <?php try {
        $logado = isset($_SESSION['logado'])?$_SESSION['logado']:'{"usuario_logado":[]}'; $logado = json_decode($logado, true); print_r($logado['usuario_logado']);
    } catch (Exception $e) {
        echo '{"usuario_logado":[]}';
    }?>
...
function listarCtrl($stateParams, $scope , $http , $httpParamSerializer ,  SweetAlert , $state ) {
            // If we got here from a url of /contacts/42
                ...
                $scope.apagar = function(entidade , $index){
                    ...
                    //$http.post('/api/'+$scope.entity , entidade);
                        ...
                        SweetAlert.swal({
                            ...
                            function(isConfirm){
                                  if (isConfirm) {
                                    $http.delete( '/api/'+$scope.entity.replace("_", "-")+"/"+entidade.id).then(function( data ) { 
                                                console.log(data);
                                               // $state.go('adminView.listar');
                                                    //$state.go('emergenciaView.emergencia')
                                        });
                                    $scope.list.splice($index,1);
                                    SweetAlert.swal("Apagou", "Seu arquivo foi removido com sucesso :)", "error");
                                  } else {
                                    SweetAlert.swal("Cancelado", "Your imaginary file is safe :)", "error");
                                  }});
                      
                       // window.location.href = "/admin/listar/"+$scope.entity+"#/interna/listar/"+$scope.entity;     
                    }


                    $scope.dupe = function(entidade){
                        $scope.duplicado = angular.copy(entidade);
                        $scope.duplicado.id = undefined;


                        //$http.post('/api/'+$scope.entity , entidade);
                        $.post( '/api/'+$scope.entity.replace("_", "-"), $httpParamSerializer($scope.duplicado) ).done(function( data ) {
                            SweetAlert.swal({
                              title: $scope.entity.replace("_", "-")+" Duplicado !",
                              text: "Essa janela vai fechar em 1 segundo.",
                              timer: 1000,
...
function editarCtrl($stateParams, $scope , $http, $httpParamSerializer ,  SweetAlert , $state) {
            ...
            // If we got here from a url of /contacts/42
                ...
                if(isset($entidade)){
	               if(isset($id)){
                	...
                	//$http.post('/api/'+$scope.entity , entidade);
                	/*$http.put('/api/'+$scope.entity.replace("_", "-")+"/"+entidade.id, {params:$httpParamSerializer(entidade)}).then(function( data ) {
					    alert( "Data Loaded: " + data );
					  });*/
                        var data = $httpParamSerializer(entidade),config='contenttype';
                      $http.put( '/api/'+$scope.entity.replace("_", "-")+"/"+entidade.id+"?"+data ).then(function( data ) {
                        //alert( "salvou : " + JSON.stringify(data)  );

                     
                            SweetAlert.swal({
                                    title: "Salvou",
                                    text: "Editamos os dados no banco de dados",
                                    type: "success",
                                    timer: 2000,
                                    showCancelButton: true,
                                    confirmButtonColor: "#DD6B55",
                                    confirmButtonText: "Ir para listagem de "+$scope.entity+"!",
                                    cancelButtonText: "Não, continuar editando!",
                                    closeOnConfirm: false,
                                    closeOnCancel: false },
                                function (isConfirm) {
                                    if (isConfirm) {
                                        SweetAlert.swal("Vamos pra area das listagem!", "Clique para continuar.", "success");
                                       // $state.go('adminView.listar');
                                        //$state.go('emergenciaView.emergencia')
                                        window.location.href = "/admin/listar/"+$scope.entity+"#/interna/listar/"+$scope.entity;
                                    } else {
                                        SweetAlert.swal("Ok", "Vamos continuar editando :)", "info");
                                    }
                                });
                	...
                	$.post( '/api/'+$scope.entity.replace("_", "-"), $httpParamSerializer(entidade) ).done(function( data ) {
					    SweetAlert.swal({
                                    title: "Salvou",
                                    text: "Salvamos os dados no banco de dados",
                                    type: "success",
                                    timer: 2000,
                                    showCancelButton: true,
                                    //confirmButtonColor: "#DD6B55",
                ...
                $scope.apagar = function(entidade){
                    ...
                    $http.delete( '/api/'+$scope.entity.replace("_", "-")+"/"+entidade.id).then(function( data ) {
                        SweetAlert.swal({
                                    title: "Removeu",
                                    text: "removemos os dados no banco de dados",
                                    type: "success",
                                    timer: 2000,
                                    showCancelButton: true,
                                    //confirmButtonColor: "#DD6B55",
...
function formCtrl($scope , $http ,$rootScope) {
    try {
        ...
        //var bt = [];
        $scope.total_pertences = pertences.length;
        $scope.pertences = pertences;

        for (var i = 0; i <= $scope.total_pertences; i++) {
            if(pertences[i] != undefined){
...
function fileController($scope, Upload , $http , $stateParams) {
    ...
    $http.get('/file/list/'+ $scope.entity+'/'+ $scope.entity_id)
           .then(function(response ) {
    ...
    $scope.upload = function (file) {
        Upload.upload({
            url: '/upload/file',
            data: {file: file, 'entidade': $scope.entity, 'id': $scope.entity_id, 'user_id': 1}
        }).then(function (resp) {
            ...
            alert('progress: ' + progressPercentage + '% ' + evt.config.data.file.name);
        });
    };
 ...
 */
function emergenciaCtrl($scope, $rootScope, $state, $timeout) {

    var simulatingGadgetFunction = function() {
        $timeout(function() {
            $state.go('emergenciaView.emergencia')
            $rootScope.bebe.status = 1;
            $timeout(function() {
                $rootScope.bebe.status = 2;
                var alertaMedio = new Audio([appPath + '/audio/alerta-medio.mp3']);
                alertaMedio.play();


                $timeout(function() {
                $rootScope.bebe.status = 3;
                var alertaGrave = new Audio([appPath + '/audio/alerta-grave.mp3']);
                alertaGrave.play()
                }, 3000)
            }, 3000)
        }, 3000)
...
function agendaCtrl($scope) {
    ...
    $scope.showFormToAddNewTask = function() {
      if($scope.addNewTask) {
        $scope.addNewTask = false;
      } else {
    ...
    $scope.selectTypeOfTask = function(data) {
        $scope.addPontual = false;
        $scope.addRecorrente = false;
        if(data === 'pontuais') {
            $scope.addPontual = true;
        } else {
            $scope.addRecorrente = true;
        }
    }
    $scope.AddNewTaskFunction = function(data) {
        console.log(data)
...
function modalDemoCtrl($scope, $uibModal) {
    ...
    $scope.open = function () {
        ...
        var modalInstance = $uibModal.open({
            templateUrl: appPath + 'views/modal_example.html',
    ...
    $scope.open1 = function () {
        var modalInstance = $uibModal.open({
            templateUrl: appPath + 'views/modal_example1.html',
            controller: ModalInstanceCtrl
        });
    };
    ...
    $scope.open2 = function () {
        var modalInstance = $uibModal.open({
            templateUrl: appPath + 'views/modal_example2.html',
            controller: ModalInstanceCtrl,
    ...
    $scope.open3 = function (size) {
        var modalInstance = $uibModal.open({
            templateUrl: appPath + 'views/modal_example3.html',
            size: size,
            controller: ModalInstanceCtrl
        });
    };
...
function notifyCtrl($scope, notify) {
    ...
    $scope.inspiniaTemplate = appPath + 'views/common/notify.html';
...
function idleTimer($scope, Idle, notify) {
    ...
    $scope.$on('IdleStart', function () {
        notify({
            ...
            templateUrl: appPath + 'views/common/notify.html'
        });
